...
function resetJournalForm() {
    ...
    customFieldOptionsDiv.style.display = "none";
...
async function openEditJournalModal() {
    ...
    try {
        ...
        journalHasCustomFieldCheckbox.checked = journal.has_custom_field || false;

        if (journal.has_custom_field) {
            customFieldOptionsDiv.style.display = "block";
            journalCustomFieldNameInput.value = journal.custom_field_name || "";
            journalCustomFieldOptionsTextarea.value = journal.custom_field_options ? journal.custom_field_options.join("\n") : "";
        }
...
async function saveJournal() {
    ...
    const data = {
        ...
        has_custom_field: journalHasCustomFieldCheckbox.checked
    };

    // Wenn ein benutzerdefiniertes Feld aktiviert ist, füge die Einstellungen hinzu
    if (data.has_custom_field) {
        data.custom_field_name = journalCustomFieldNameInput.value.trim();
        if (!data.custom_field_name) {
            alert("Bitte geben Sie einen Namen für das benutzerdefinierte Feld ein.");
            return;
        }

        // Optionen als Array speichern
        const options = journalCustomFieldOptionsTextarea.value.split("\n")
            .map(line => line.trim()).filter(line => line);
        data.custom_field_options = options;
    }
...
async function loadStrategies() {
    try {
        strategiesData = await apiRequest("/strategies");
        updateStrategyDropdown();
    } catch (error) {
        console.error("Failed to load strategies:", error);
        strategiesData = []; // Reset on error
    }
}

function updateStrategyDropdown() {
    // Leere das Dropdown
    entryStrategySelect.innerHTML = "<option value=\"\">-- Auswählen --</option>";
    ...
    strategiesData.forEach(strategy => {
        ...
        entryStrategySelect.appendChild(option);
    });
}

function resetStrategyForm() {
    newStrategyNameInput.value = "";
}
...
async function saveStrategy() {
    const name = newStrategyNameInput.value.trim();
    if (!name) {
        alert("Strategiename darf nicht leer sein.");
        return;
    }

    try {
        const newStrategy = await apiRequest("/strategies", "POST", { name });
        strategiesData.push(newStrategy);
        updateStrategyDropdown();
        // Setze im Eintrag die neue Strategie
        entryStrategySelect.value = newStrategy.name;
        closeModal("strategy-modal");
    } catch (error) {
        alert(`Fehler beim Speichern der Strategie: ${error.message}`);
    }
}
...
function renderEntries(entries) {
    ...
    entries.forEach(entry => {
        ...
        div.innerHTML = `
            ...
            ${currentJournalHasCustomField && entry.custom_field_value ?
              `<p><strong>${currentJournalCustomFieldName}:</strong> ${entry.custom_field_value}</p>` : ""}
        `;
...
function resetEntryForm() {
    ...
    entryPositionTypeSelect.value = "";
    entryStrategySelect.value = "";
    entryInitialRrInput.value = "";
    entryPnlInput.value = "";
    ...
    // Benutzerdefiniertes Feld zeigen/verstecken
    if (currentJournalHasCustomField) {
        customFieldDiv.style.display = "block";
        customFieldLabel.textContent = `${currentJournalCustomFieldName}:`;

        // Dropdown-Optionen füllen
        entryCustomFieldSelect.innerHTML = "<option value=\"\">-- Auswählen --</option>";
        currentJournalCustomFieldOptions.forEach(option => {
            const optionEl = document.createElement("option");
            optionEl.value = option;
            optionEl.textContent = option;
            entryCustomFieldSelect.appendChild(optionEl);
        });
    } else {
...
function openNewEntryModal() {
    ...
    resetEntryForm();
    loadStrategies(); // Strategien für das Dropdown laden
...
async function openEditEntryModal(entryId) {
    ...
    try {
        ...
        entrySymbolInput.value = entry.symbol || "";
        entryPositionTypeSelect.value = entry.position_type || "";
        loadStrategies().then(() => {
            entryStrategySelect.value = entry.strategy || "";
        });
        entryInitialRrInput.value = entry.initial_rr !== null ? entry.initial_rr : "";
        ...
        if (currentJournalHasCustomField) {
            ...
            entryCustomFieldSelect.value = entry.custom_field_value || "";
        } else {
            customFieldDiv.style.display = "none";
        }
...
async function saveEntry() {
    ...
    const data = {
        symbol: entrySymbolInput.value.trim() || null,
        position_type: entryPositionTypeSelect.value || null,
        strategy: entryStrategySelect.value || null,
        initial_rr: entryInitialRrInput.value !== "" ? parseFloat(entryInitialRrInput.value) : null,
    ...
    if (currentJournalHasCustomField) {
        data.custom_field_value = entryCustomFieldSelect.value || null;
    }
...
async function handleImageUpload() {
    ...
    const category = entryImageCategorySelect.value; // Nur noch Before oder After
...
function addImageToPreview(image) {
    ...
    if (image.category === "Before") {
        entryImagesPreviewBeforeDiv.appendChild(container);
    } else if (image.category === "After") {
...
function renderStatistics(stats) {
    ...
    // Neue Sektion für Strategie-Performance
    if (stats.strategy_performance && stats.strategy_performance.length > 0) {
        html += `<div class="stats-section">
                    <h4>Performance nach Strategie</h4>
                    <table>
                        <thead><tr><th>Strategie</th><th>Anzahl</th><th>Gewinner</th><th>Verlierer</th><th>Winrate</th><th>Durchschn. P&L</th></tr></thead>
                        <tbody>`;
        stats.strategy_performance.forEach(item => {
            html += `<tr>
                        <td>${item.strategy}</td>
                        <td>${item.count}</td>
                        <td>${item.wins}</td>
                        <td>${item.losses}</td>
                        <td><span class="${item.win_rate >= 50 ? "positive" : "negative"}">${item.win_rate.toFixed(1)}%</span></td>
                        <td><span class="${item.avg_pnl >= 0 ? "positive" : "negative"}">${item.avg_pnl.toFixed(2)}</span></td>
                     </tr>`;
        });
        html += `   </tbody>
                    </table>
...
// Strategie-Buttons
addNewStrategyBtn.addEventListener("click", openNewStrategyModal);
